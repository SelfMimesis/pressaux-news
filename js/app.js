...
function render() {
  ...
  state.questions.forEach((question, i) => {
    ...
    article.innerHTML = `<div class="q-row"><span class="q-num">${String(i + 1).padStart(2, '0')}</span><span class="q-text" tabindex="0">${escapeHtml(question)}</span><button class="icon-btn complete" aria-label="${r.completed ? 'Marcar pendiente' : 'Marcar completada'}" aria-pressed="${r.completed}">OK</button><button class="icon-btn favorite" aria-label="${r.favorite ? 'Quitar favorita' : 'Marcar favorita'}" aria-pressed="${r.favorite}">FAV</button></div><div class="note-dot">${r.note ? 'NOTES STORED' : 'NO NOTES'}</div><label class="sr-only" for="note-${i}">Notas para pregunta ${i + 1}</label><textarea id="note-${i}" class="note-area" placeholder="ENTER FIELD NOTES...">${escapeHtml(r.note)}</textarea>`;
    list.append(article);
  });
